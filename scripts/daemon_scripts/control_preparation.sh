...
get_disease_classes.py $control_genes_folder/zampieri/processed_data/disease_genes_disorderclass 30 > $control_genes_folder/zampieri/processed_data/disgroup_gene
# Getting negatives
...
desaggregate_column_data -i $control_genes_folder/menche/data/menche_bench.tsv -x 2 -s ";" | standard_name_replacer -i - -I ./translators/symbol_HGNC -c 2 -u > menche_symbol
cut -f 1 menche_symbol | tr -s " " "_" | uniq -c | awk '{FS=" ";OFS="\t"}{if ($1 > 19){print $2,$1}}' | cut -f 1 | tr -s "_" " " > menche_20
grep -w -f menche_20 menche_symbol | aggregate_column_data -i - -x 1 -a 2 > $control_genes_folder/menche/disease_gens
