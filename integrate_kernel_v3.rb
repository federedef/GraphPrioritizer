...
class Kernels
	...
	attr_accessor :kernels_comb, :kernels_in_genmatrix, :integrated_kernel
	...
	def load_kernels_by_bin_matrixes(input_matrix, input_nodes, kernels_names)
		...
		kernels_names.each.with_index do |kernel_name, pos|
			...
			@kernels_comb[kernel_name.to_sym] = [kernel, node]
	  end
	end

	def minmax_normalize

		@kernels_comb.each do |kern, value|
			matrix = value[0]
			normalize_matrix = (1.to_f/(matrix.max-matrix.min)) * matrix 
			@kernels_comb[kern][0] = normalize_matrix 
		end
	end
	...
	def kernels2generalMatrix
		...
		general_nodes = []
		@kernels_comb.each_value do |kernel| 
			general_nodes += kernel[1]
		end
		general_nodes.uniq!

		@kernels_comb.each do |kernel, value|
