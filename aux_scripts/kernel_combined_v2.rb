...
class Kernels
	...
	def load_kernels_by_bin_matrixes(input_matrix, input_nodes)

		input_matrix.each_with_index do |kernel_path, pos|
			kernel = Npy.load(kernel_path)
			node = lst2arr(input_nodes[pos])
			@kernels_raw.append([kernel, node])
	  end
	end

	def create_general_index
		@kernels_raw.each do |kernel| 
