    ...
    <h1 style="text-align:center; background-color:#ecf0f1, color: powderblue; "> Analysis of the algorithm: From rankings to prioritized genes.</h1>
        ...
        <div style="overflow: hidden; text-align:center">
                % if plotter.hash_vars.get("non_integrated_rank_cdf") is not None: 
                        ${ plotter.static_plot_main( id="non_integrated_rank_cdf", header=True, row_names=False, var_attr=[0,1,2,3], fields =[4,5,6],
                                        plotting_function= lambda data, plotter_list: plotter_list["sns"].FacetGrid(data, col_wrap=4, col="annot", hue="kernel", aspect=1).map(plotter_list["sns"].ecdfplot, 'rank').fig.suptitle("CDF for Non-Zero Scores Generated by Embeddings before Integration"))}
                % endif
        </div>

        <div style="overflow: hidden; text-align:center">
                % if plotter.hash_vars.get("non_integrated_rank_measures") is not None: 
                        ${ plotter.static_plot_main( id="non_integrated_rank_measures", header=True, row_names=False, var_attr=[0,1,2], fields =[3,4,5,6,7,8],
                                        plotting_function= lambda data, plotter_list: plotter_list["sns"].FacetGrid(data, col_wrap=4, col="annot", hue="kernel", aspect=1).map(plotter_list["sns"].lineplot, 'fpr', 'tpr').fig.suptitle("ROC curves for embeddings before Integration"))}
                % endif
        </div>
        ...
        <div style="overflow: hidden; text-align:center">
                % if plotter.hash_vars.get("integrated_rank_measures") is not None: 
                        ${ plotter.static_plot_main( id="integrated_rank_measures", header=True, row_names=False, var_attr=[0,1,2], fields =[3,4,5,6,7,8],
                                        plotting_function= lambda data, plotter_list: plotter_list["sns"].FacetGrid(data, col_wrap=2, col="integration", hue="kernel", aspect=1).map(plotter_list["sns"].lineplot, 'fpr', 'tpr').fig.suptitle("ROC curves for embeddings before Integration"))}
                % endif
        </div>
</div>
